<template>
  <div>
    ...
    <PaginationWidget :pages="pagination" @emit-pages="getProducts"></PaginationWidget>
    <!-- ProductModal -->
    <ProductModal @update-product="updateProduct" :product="tempProduct" :isNew="isNew"
      ref="productModal" />
    <!-- DelModal -->
...
import PaginationWidget from '@/components/dashboard/PaginationWidget.vue';
...
export default {
  ...
  methods: {
    ...
    updateProduct(item) {
      ...
      let status = '新增產品';
      if (!this.isNew) {
        api = `${VITE_APP_URL}/v2/api/${VITE_APP_PATH}/admin/product/${this.tempProduct.id}`;
        httpMethod = 'put';
        status = '更新產品';
      }
      const productComponent = this.$refs.productModal;
